...
const interScene = preload("res://Scenes/LoadingScene.tscn");
...
func fade_out(from, to, duration: float, color: Color, sp: Vector2 = Vector2.ZERO, sa: Vector2 = Vector2.ZERO) -> void:
	...
	var new_scenee = interScene.instance()
	get_tree().get_root().add_child(new_scenee)
	if from != null:
		from.queue_free()

	get_tree().set_current_scene(new_scenee)
	...
	if new_scenee != null:
		new_scenee.queue_free()
	if sp != Vector2.ZERO:
		var player = get_tree().get_root().find_node("Player", true, false);
		var cam = player.get_parent().find_node("Camera2D");
		if player != null:
			player.set("position", sp);
			cam.set("position", sp);
			player.set("starting_angle", sa);
			#print(player.get("starting_angle"))
